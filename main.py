# main.py
# Robust Telegram Quiz Bot — final version
# Requirements: python-telegram-bot[job-queue,rate-limiter]>=21.0,<22
# Optional: psutil for memory logging (pip install psutil)
...
# Accept several env names for token
...
# LOCK_SECONDS: interpret env. Accept "QUESTION_TIME" or integer
...
# Optional debug and memory cap
DEBUG_UPDATES = _bool("DEBUG_UPDATES", False)
MAX_RSS_MB = _int("MAX_RSS_MB", 0, 0, 65536)  # 0 disables watcher

log.info("CONFIG: TOKEN=%s QUESTION_TIME=%s LOCK_SECONDS=%s DELAY_NEXT=%s TICK_SECONDS=%s ADMINS_ONLY=%s DEBUG_UPDATES=%s MAX_RSS_MB=%s",
         ("SET" if TOKEN else "MISSING"), QUESTION_TIME, LOCK_SECONDS, DELAY_NEXT, TICK_SECONDS, ADMINS_ONLY, DEBUG_UPDATES, MAX_RSS_MB)
log_mem("startup")
...
async def _tg(desc: str, coro, *args, **kwargs):
    ...
    while True:
        ...
        try:
            ...
            if attempts >= 3:
                log.warning("%s: giving up after RetryAfter", desc); return None
            ...
            log.debug("%s: BadRequest: %s", desc, e); return None
        except Exception:
...
# ---------- helpers ----------
def points_for_elapsed(elap: float) -> int:
    if elap >= LOCK_SECONDS:
        return 0
    return int(round((max(0.0, LOCK_SECONDS - elap) / LOCK_SECONDS) * POINTS_MAX))
...
async def ask_question(context: ContextTypes.DEFAULT_TYPE, st: GameState):
    ...
    m = await _tg("ask.send", context.bot.send_message, chat_id=st.chat_id,
                  ...
                  parse_mode=ParseMode.HTML)
    if not m:
        log.error("ask_question: failed to send; finishing"); await finish_quiz(context, st); return
    ...
    log.info("ask_question: scheduled tick and close for chat=%s qidx=%d", st.chat_id, st.q_index)
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("/startquiz <mode> <size>\n/stopquiz\n/ping\n/help")
...
async def do_startquiz(context: ContextTypes.DEFAULT_TYPE, chat_id: int, starter_user_id: int, mode: str, length: int):
    ...
    try:
        ...
        log.exception("do_startquiz: failed to load questions"); await _tg("start.err", context.bot.send_message, chat_id=chat_id, text=f"Failed to load questions: {e}"); return
...
def build_app() -> Application:
    ...
    # optional debug: log raw updates
    if DEBUG_UPDATES:
        async def dump_update(update: Update, context: ContextTypes.DEFAULT_TYPE):
            try:
                u = update.to_dict() if hasattr(update, "to_dict") else str(update)
                log.info("RAW UPDATE: %s", u)
            except Exception:
                log.exception("dump_update failed")
        app.add_handler(MessageHandler(filters.ALL, dump_update))
    return app

# ---------- keepalive & mem watcher ----------
def start_keepalive_if_needed():
    port_env = os.getenv("PORT")
    if not port_env:
        log.info("No $PORT detected — not starting keepalive"); return
    try:
        port = int(port_env)
    except Exception:
        log.warning("Invalid PORT=%r; skipping keepalive", port_env); return
    ...
    def run():
        try:
            log.info("Keepalive HTTP listening on 0.0.0.0:%d", port)
...
def start_mem_watcher_if_needed():
    ...
    if not psutil:
        log.warning("MAX_RSS_MB set but psutil not installed; disabling mem watcher")
        return
    def loop():
        try:
            p = psutil.Process(os.getpid())
            while True:
                ...
                if rss > MAX_RSS_MB:
                    log.warning("Memory %.1fMB > %.1fMB => exiting for restart", rss, MAX_RSS_MB)
                    os._exit(1)
                time.sleep(5)
        except Exception:
            log.exception("mem watcher crashed"); time.sleep(5)
