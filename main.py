# Telegram Quiz Bot — "previous/full" version, no env vars for timing
...
# --------- Fixed settings (no env needed) ----------
QUESTION_TIME = 10          # seconds per question
DELAY_NEXT = 5              # seconds between questions (classic behavior)
TICK_SECONDS = 1.0          # countdown edit interval (classic = 1s)
POINTS_MAX = 100            # for per-question scoring
...
# --------- Safe Telegram call with retries ----------
...
def cancel_jobs_for_chat(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    try:
        for job in context.job_queue.jobs():
            ...
            if name.startswith(f"tick:{chat_id}:") or name.startswith(f"close:{chat_id}:"):
                job.schedule_removal()
    except Exception as e: log.warning("job cleanup error: %s", e)

# --------- Jobs (countdown & closing) ---------
...
async def close_question(context: ContextTypes.DEFAULT_TYPE):
    ...
    # Recap
    qidx = st.q_index; q = st.questions[qidx]; per = st.per_q_answers.get(qidx, {})
    ...
    # free per-question memory
    st.per_q_answers.pop(st.q_index, None); st.answered_now.pop(st.q_index, None)

    # Next question
    context.job_queue.run_once(next_question, when=max(0.1, DELAY_NEXT), data={"chat_id": chat_id})
...
async def on_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try: await q.answer(f"You chose: {st.questions[st.q_index].options[opt]}\n"+("✅ Correct" if is_correct else "❌ Locked"), show_alert=False)
    except: pass
...
def build_app() -> Application:
    ...
    # Optional rate limiter (won't crash if the extra isn't present)
    try:
        from telegram.ext import AIORateLimiter
        builder = builder.rate_limiter(AIORateLimiter())
    except Exception as e:
        log.info("Rate limiter not available; continuing without it: %s", e)
